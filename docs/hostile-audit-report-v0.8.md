# Hostile Audit Report for Minimum Kernel Footing v0.8
...
- Sequence preflight report (`PreflightReport`, `preflight_sequence`).
- Test suite and failure behavior.
- Example binary behavior.

The audit boundary matches the certification boundary defined in `docs/hostile-audit-packet-v0.md` (updated for v0.8).

---

## 2. Audit Procedure

1. Ran `cargo test` and confirmed all 43 tests pass.
...
13. Inspected `src/verification/mod.rs` and confirmed `preflight_sequence()` correctly runs on a clone, stops at first unlawful event, reports per-event results, and leaves the original unchanged.
14. Attempted to identify hidden mutation paths accessible from outside the crate.
15. Attempted to identify ways to tamper with `MovementMemory` without detection.
16. Reviewed the hash function for suitability.
...
`diff()` correctly compares the two realities and reports which components are identical.
...
### 3.14 Sequence preflight report

`preflight_sequence()` correctly:
- Processes events in order.
- Stops at the first unlawful event.
- Reports `results` booleans for each processed event.
- Reports `transition_count` as the number of accepted movements.
- Returns `final_state` only if the full sequence is lawful.
- Leaves the original `Reality` unchanged.

Tests cover fully lawful sequences, partially lawful sequences, and original non-mutation.

**Result: Pass**

### 3.15 ProofResult runtime checks
...
The kernel is ready for external certification as **Minimum Kernel Footing v0.8**.
