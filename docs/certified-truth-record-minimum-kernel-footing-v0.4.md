# Certified Truth Record: Minimum Kernel Footing v0.4

## Certification Boundary

The following are certified as **Minimum Kernel Footing v0.4**:
...
- Modules:
  ...
  - `src/verification/` — `Inspection`, `Replay`, `Continuity`, `DriftCheck`, `ProofResult`, `MovementError`, `VerificationReport`, `detect_drift`, `PlannedSequence`, `plan_sequence`, `RealityFingerprint`, `fingerprint()`, `IntegrityReport`, `integrity_report()`
...
- Control documents:
  ...
  - `docs/hostile-audit-report-v0.4.md`
...
- **Tests**: 31 unit tests pass (`cargo test`)
...
- **Hostile audit report**: `docs/hostile-audit-report-v0.4.md` found no blockers.
...
3. This certification applies only to the minimum kernel footing v0.4, not to future expansion.
...
The kernel is certified as **Minimum Kernel Footing v0.4** within the above boundary only.
