# Certified Truth Record: Minimum Kernel Footing v0.6

## Certification Boundary

The following are certified as **Minimum Kernel Footing v0.6**:
...
- Modules:
  ...
  - `src/verification/` — `Inspection`, `Replay`, `Continuity`, `DriftCheck`, `ProofResult`, `MovementError`, `VerificationReport`, `detect_drift`, `PlannedSequence`, `plan_sequence`, `RealityFingerprint`, `fingerprint()`, `IntegrityReport`, `integrity_report()`, `would_accept()`, `SimulationReport`, `simulate_sequence`
...
- Control documents:
  ...
  - `docs/hostile-audit-report-v0.6.md`
...
- **Tests**: 37 unit tests pass (`cargo test`)
...
- **Hostile audit report**: `docs/hostile-audit-report-v0.6.md` found no blockers.
...
4. This certification applies only to the minimum kernel footing v0.6, not to future expansion.
...
The kernel is certified as **Minimum Kernel Footing v0.6** within the above boundary only.
