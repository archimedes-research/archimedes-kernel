# Certified Truth Record: Minimum Kernel Footing v0.7

## Certification Boundary

The following are certified as **Minimum Kernel Footing v0.7**:
...
- Modules:
  ...
  - `src/verification/` — `Inspection`, `Replay`, `Continuity`, `DriftCheck`, `ProofResult`, `MovementError`, `VerificationReport`, `detect_drift`, `PlannedSequence`, `plan_sequence`, `RealityFingerprint`, `fingerprint()`, `IntegrityReport`, `integrity_report()`, `would_accept()`, `SimulationReport`, `simulate_sequence`, `RealityDiff`, `diff()`
...
- Control documents:
  ...
  - `docs/hostile-audit-report-v0.7.md`
...
- **Tests**: 40 unit tests pass (`cargo test`)
- **Example**: `cargo run --example demo` produces the expected output, including:
  ...
  - Reality Diff section.
- **Hostile audit report**: `docs/hostile-audit-report-v0.7.md` found no blockers.
...
4. The fingerprint currently excludes mutable current boundary and law; it includes birth boundary, birth law, current state, and memory hash. A change only to current boundary or law will not alter the fingerprint, though `diff()` and `drift_check()` will detect it.
5. This certification applies only to the minimum kernel footing v0.7, not to future expansion.
...
The kernel is certified as **Minimum Kernel Footing v0.7** within the above boundary only.
